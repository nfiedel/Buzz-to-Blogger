...
class InputHandler(webapp.RequestHandler):
  ...
  def get(self):
    ...
    # TODO: Use the db store to check if we've asked for this subscription (hub.topic == feedUrl)
  ...
  def post(self):
    ...
    feedUrl = ''
    for node in dom.getElementsByTagNameNS(ATOM_NS, 'link'):
        if 'self' == node.getAttribute('rel'):
            feedUrl = node.getAttribute('href')
    logging.info('found feedUrl=%s' % feedUrl)
  ...
  def old_post(self):
    ...
    #no need for datastore during devel
    #db.put(update_list)
    memcache.add("last_update_list_global", update_list, 3600)
    self.response.set_status(200)
...
class DebugHandler(webapp.RequestHandler):
  ...
  def get(self):
    logging.info("debug handler called")
...
class ViewHandler(webapp.RequestHandler):
  ...
  def get(self):
    ...
    context = dict(entries=memcache.get("last_update_list_global"))
